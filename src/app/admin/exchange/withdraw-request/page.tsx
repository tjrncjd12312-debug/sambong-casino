...
export default function WithdrawRequestPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch withdraw requests");
    } finally {
      setLoading(false);
    }
  ...
  const handleApprove = async (id: string) => {
    if (!confirm("이 환전 신청을 승인하시겠습니까?")) return;
    setProcessing(id);
    try {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-sm text-white/80">
        환전 대기 : <span className="text-orange-400 font-bold">{data.length}건</span>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-green-900/15 hover:bg-transparent">
                  {["#", "회원ID", "닉네임", "출금요청금액", "현재 잔고", "요청일시", "예금주", "은행명", "출금계좌정보", "소속", "메모", "승인", "거부"].map((h) => (
                ...
                {loading ? (
                  ...
                  data.map((item, idx) => (
                    <TableRow key={item.id} className="border-green-900/10 hover:bg-white/[0.02]">
                      ...
                      <TableCell className="text-center text-xs text-white/70">{item.bank_holder || item.member?.bank_holder || "-"}</TableCell>
                      <TableCell className="text-center text-xs text-white/70">{item.bank_name || item.member?.bank_name || "-"}</TableCell>
                      <TableCell className="text-center text-xs text-white/60 font-mono">{item.bank_account || item.member?.bank_account || "-"}</TableCell>
