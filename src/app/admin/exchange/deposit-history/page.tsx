  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-sm text-white/80">
        총입금 : <span className="text-green-400 font-bold">{summary.totalAmount.toLocaleString()}</span>
        (<span className="text-green-400">{summary.totalCount}건</span>)
      </div>
                  ...
                  filteredData.map((item, idx) => (
                    <TableRow key={item.id} className="border-green-900/10 hover:bg-white/[0.02]">
                      ...
                      <TableCell className="text-center text-xs">
                        <span className="px-2 py-0.5 rounded bg-green-600/20 text-green-400 text-[10px] font-bold">충전</span>
                      </TableCell>
                      <TableCell className="text-center text-xs text-green-400 font-bold">
                        {Number(item.amount).toLocaleString()}
                        {Number(item.bonus_amount || 0) > 0 && (
                          <span className="text-yellow-400 text-[10px] block">+{Number(item.bonus_amount).toLocaleString()}</span>
                        )}
                      </TableCell>
                      <TableCell className="text-center text-xs text-white/60">-</TableCell>
