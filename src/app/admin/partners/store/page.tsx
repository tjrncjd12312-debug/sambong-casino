...
export default function StorePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (json.data) {
        ...
        const stores = allPartners.filter((p) => p.level === "store");
        const mapped: StoreRow[] = stores.map((p, i) => {
          const descendants = getDescendants(p.id, allPartners);
          return {
            no: i + 1,
            id: p.id,
            bonsa: findAncestorByLevel(p.parent_id, "head", allPartners),
            bubon: findAncestorByLevel(p.parent_id, "sub_head", allPartners),
            chongpan: findAncestorByLevel(p.parent_id, "distributor", allPartners),
            name: p.username,
  ...
  const filteredRows = searchKeyword
    ? sortedRows.filter((row) => {
        ...
        return row.name.toLowerCase().includes(kw) || row.nickname.toLowerCase().includes(kw) || row.bonsa.toLowerCase().includes(kw) || row.bubon.toLowerCase().includes(kw) || row.chongpan.toLowerCase().includes(kw);
      })
    : sortedRows;
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-green-900/15 hover:bg-transparent">
                ...
                <TableHead className="text-center text-xs text-white/70 font-bold py-3">총판</TableHead>
                <TableHead className="text-center text-xs text-white/70 font-bold py-3">매장</TableHead>
                <TableHead className="text-center text-xs text-white/70 font-bold py-3">그룹설정</TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow className="border-green-900/10">
                  <TableCell colSpan={17} className="text-center py-32">
                ...
                <TableRow className="border-green-900/10">
                  <TableCell colSpan={17} className="text-center py-32 text-white/30 text-sm">
                    매장 데이터가 없습니다
                  </TableCell>
                </TableRow>
              ) : (
                paginatedRows.map((row) => (
                  <TableRow key={row.no} className="border-green-900/10 hover:bg-white/[0.02] transition-colors">
                    ...
                    <TableCell className="text-center text-xs text-white/80">{row.bubon}</TableCell>
                    <TableCell className="text-center text-xs text-white/80">{row.chongpan}</TableCell>
                    <TableCell className="text-center">
                      <div className="text-sm font-bold text-white">{row.name}</div>
                      <div className="text-xs text-green-400">({row.nickname})</div>
                    </TableCell>
                    <TableCell className="text-center text-xs text-white/80">{row.group}</TableCell>
