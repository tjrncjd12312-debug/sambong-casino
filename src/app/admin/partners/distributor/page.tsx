...
export default function DistributorPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (json.data) {
        ...
        const distributors = allPartners.filter((p) => p.level === "distributor");
        const mapped: DistributorRow[] = distributors.map((p, i) => {
          ...
          return {
            ...
            maejangCount: countByLevel(descendants, "store"),
            members: 0,
  ...
  const filteredRows = searchKeyword
    ? sortedRows.filter((row) => {
        ...
        return row.name.toLowerCase().includes(kw) || row.nickname.toLowerCase().includes(kw) || row.bonsa.toLowerCase().includes(kw) || row.bubon.toLowerCase().includes(kw);
      })
    : sortedRows;
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-green-900/15 hover:bg-transparent">
                ...
                <TableHead className="text-center text-xs text-white/70 font-bold py-3">매장수</TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow className="border-green-900/10">
                  <TableCell colSpan={16} className="text-center py-32">
                ...
                <TableRow className="border-green-900/10">
                  <TableCell colSpan={16} className="text-center py-32 text-white/30 text-sm">
                    총판 데이터가 없습니다
                  </TableCell>
                </TableRow>
              ) : (
                paginatedRows.map((row) => (
                  <TableRow key={row.no} className="border-green-900/10 hover:bg-white/[0.02] transition-colors">
                    ...
                    <TableCell className="text-center text-xs text-white/80">{row.maejangCount}</TableCell>
